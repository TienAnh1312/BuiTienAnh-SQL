...
AS BEGIN SELECT * FROM VATTU ORDER BY TenVTu ASC; END; 
GO
...
CREATE PROCEDURE spud_LayDanhSach_NHACC
	@mancc VARCHAR(10) = NULL --thay đổi dữ liệu của @mancc thành varchar
AS BEGIN
	IF @mancc IS NULL
	BEGIN
		SELECT * FROM NHACC;
...
AS
BEGIN
	-- cập nhập thông tin trong bảng VATTU
	UPDATE VATTU SET
		tenvtu = COALESCE(@tenvtu, tenvtu), --nếu tham số null thì giữ nguyên giá trị cũ
		dvtinh = COALESCE(@donvitinh, dvtinh), --tương tụ với đơn vị tính
		phantram = COALESCE(@phantram, phantram) -- tương tự với phần trăm
	WHERE mavtu = @mavtu; END;

	--Sửa đổi tên vật tư, đơn vị tính và phần trăm cảu vật tư có mã 'V001'
EXEC spud_VATTU_Sua @mavtu = 'DD01', @tenvtu = 'Vật tư mới', @donvitinh = 'kg', @phantram = 10.50;

	--chỉ sửa phần trăm của vật tư có mã 'V002'
EXEC spud_VATTU_Sua @mavtu = 'DD01', @phantram = 15.50;

	--không thay đổi bất kỳ giá trị nào, chỉ cần sửa tên vật tư
EXEC spud_VATTU_Sua @mavtu = 'V003', @tenvtu = 'Vật tư đã thay đổi';